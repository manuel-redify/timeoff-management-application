...
const { sorter, get_sorted_countries } = require("../../util");
  ...
  req.user
    ...
    .then(async function (company) {
      res.render('user_add', {
        ...
        roles: await company.sequelize.models.Role.findAll({ where: { companyId: company.id } }),
        teams: await company.sequelize.models.Team.findAll({ where: { companyId: company.id } }),
        projects: await company.sequelize.models.Project.findAll({ where: { companyId: company.id } }),
        areas: await company.sequelize.models.Area.findAll({ where: { companyId: company.id } }),
      });
    });
});
...
router.post('/add/', function (req, res) {
  ...
  req.user
    ...
    .then(() => UserImporter.add_user({
      ...
      country: new_user_attributes.country,
      contract_type: new_user_attributes.contract_type,
      role_id: new_user_attributes.role_id,
      area_id: new_user_attributes.area_id,
      team_id: new_user_attributes.team_id,
      project_id: new_user_attributes.projects,
    }))
    ...
    .then(new_user => Email.promise_add_new_user_email({
      ...
      .catch(error => {
        ...
        req.session.flash_error('User added but failed to send welcome email');
      }))

    .then(function () {
      const flash = req.session.flash || {};
      const errors = flash.errors || [];
      const onlyEmailError = errors.length === 1 && errors[0].indexOf('welcome email') !== -1;

      if (!req.session.flash_has_errors() || onlyEmailError) {
      ...
      return employee.promise_schedule_I_obey()
        .then(async function () {
          res.render('user_details', {
            ...
            projects: await company.sequelize.models.Project.findAll({ where: { companyId: company.id } }),
            areas: await company.sequelize.models.Area.findAll({ where: { companyId: company.id } }),
            userRoles: await employee.getRoles(),
            userTeams: await employee.getTeams(),
            userProjects: await employee.getProjects(),
            userAreas: await employee.promise_areas(),
          });
        });
    })
    .catch(function (error) {
  ...
  Promise.try(function () {
    ...
    .then(function () {
      ...
      employee
        ...
        .then(() => {
          const role_id = (new_user_attributes.role_id && String(new_user_attributes.role_id).trim() !== '') ? parseInt(new_user_attributes.role_id) : null;
          const area_id = (new_user_attributes.area_id && String(new_user_attributes.area_id).trim() !== '') ? parseInt(new_user_attributes.area_id) : null;
          const team_id = (new_user_attributes.team_id && String(new_user_attributes.team_id).trim() !== '') ? parseInt(new_user_attributes.team_id) : null;
          const projects_data = new_user_attributes.projects;

          return Promise.all([
            employee.update({ DefaultRoleId: role_id }),
            employee.setRoles(role_id ? [role_id] : []),
            employee.setTeams(team_id ? [team_id] : []),
            // Manual project update to handle join table attributes
            model.UserProject.destroy({ where: { user_id: employee.id } })
              .then(() => Promise.map(projects_data, p => model.UserProject.create({
                user_id: employee.id,
                project_id: p.id,
                role_id: p.role_id
              })))
            ...
            if (role_id) {
              return model.UserRoleArea.update(
                { area_id: area_id },
                { where: { user_id: employee.id, role_id: role_id } }
              );
            }
          });
        })
    ...
    include: [
      {
        ...
        include: [
          ...
          { model: model.Role, as: 'roles' },
          { model: model.Area, as: 'areas' },
...
function promise_user_list_data_for_rendering(args) {
  ...
  const usersInfoForRendering = users_info.map(ui => ({
    ...
    department_id: ui.user_row.department ? ui.user_row.department.id : (ui.user_row.DepartmentId || null),
    department_name: (ui.user_row.department ? ui.user_row.department.name : null)
      || (ui.user_row.Department ? ui.user_row.Department.name : null)
      || (ui.user_row.DepartmentId && company.departments ? (company.departments.find(d => d.id === ui.user_row.DepartmentId) || {}).name : null)
      || 'N/A',
    role_name: (ui.user_row.roles || []).map(r => r.name).join(', ') || 'N/A',
    area_name: (ui.user_row.areas || []).map(a => a.name).join(', ') || 'N/A',
    contract_type: ui.user_row.contract_type || 'N/A',
  ...
  const sortedUsersInfoForRendering = usersInfoForRendering
    .sort((a, b) => sorter(a.user_name + a.user_lastname, b.user_name + b.user_lastname));
...
function get_and_validate_user_parameters(args) {
  ...
  var name = validator.trim(req.body['name']),
    ...
    country = validator.trim(req.body['country']);
  ...
  var attributes = {
    ...
    country: country,
    contract_type: req.body['contract_type'] || 'Employee',
    role_id: (req.body['role'] && String(req.body['role']).trim() !== '') ? parseInt(req.body['role']) : null,
    area_id: (req.body['area'] && String(req.body['area']).trim() !== '') ? parseInt(req.body['area']) : null,
    team_id: (req.body['team'] && String(req.body['team']).trim() !== '') ? parseInt(req.body['team']) : null,

    // Normalize projects to include associated roles
    projects: (function () {
      let projectIds = req.body['project'];
      if (!projectIds) {
        return [];
      }
      ...
      return projectIds.map(id => ({
        id: parseInt(id),
        role_id: (req.body['project_role_' + id] && String(req.body['project_role_' + id]).trim() !== '') ? parseInt(req.body['project_role_' + id]) : null
      })).filter(p => !isNaN(p.id));
    })(),
  };
