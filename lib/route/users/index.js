...
router.post('/add/', function (req, res) {
  ...
  req.user
    ...
    .then(() => UserImporter.add_user({
      ...
      project_id: new_user_attributes.project_id,
    }))
    ...
    .then(function () {
      ...
      employee
        ...
        .then(() => {
          ...
          const project_id = (new_user_attributes.project_id && String(new_user_attributes.project_id).trim() !== '') ? parseInt(new_user_attributes.project_id) : null;

          return Promise.all([
            employee.setRoles(role_id ? [role_id] : []),
            employee.setTeams(team_id ? [team_id] : []),
            employee.setProjects(project_id ? [project_id] : []),
          ]).then(() => {
            if (role_id) {
...
function get_and_validate_user_parameters(args) {
  ...
  var attributes = {
    ...
    project_id: (req.body['project'] && String(req.body['project']).trim() !== '') ? parseInt(req.body['project']) : null,
  };
