...
const express = require('express'),
  ...
  { sorter, get_sorted_countries, get_filtered_sorted_countries } = require('../util');
...
router.get('/bankholidays/', (req, res) => {
  ...
  req.user.getCompany({
    ...
    .then(async (company) => {
      // Determine unique countries assigned to employees or the company itself
      const userCountries = company.users.map(u => u.country);
      const allActiveCountryCodes = [...new Set([...userCountries, company.country])].filter(Boolean);

      // Check if we need to auto-populate any missing holidays for current year
      const holidaysByCountry = {};
      company.bank_holidays.forEach(bh => {
        const year = moment.utc(bh.date).year();
        const c = bh.country;
        if (!holidaysByCountry[c]) holidaysByCountry[c] = new Set();
        holidaysByCountry[c].add(year);
      });

      const countriesToImport = allActiveCountryCodes.filter(c =>
        !holidaysByCountry[c] || !holidaysByCountry[c].has(currentYear)
      );

      if (countriesToImport.length > 0) {
        const config_countries = config.get('countries');
        let toCreate = [];
        countriesToImport.forEach(c => {
          if (config_countries[c] && config_countries[c].bank_holidays) {
            const bhs = config_countries[c].bank_holidays.filter(bh => moment.utc(bh.date).year() === currentYear);
            bhs.forEach(bh => {
              toCreate.push({ name: bh.name, date: bh.date, companyId: company.id, country: c });
            });
          }
        });

        if (toCreate.length > 0) {
          await Models.BankHoliday.bulkCreate(toCreate);
          // Refresh company data to include new holidays
          company = await req.user.getCompany({
            scope: ['with_bank_holidays', 'order_by_bank_holidays'],
            include: [{ model: Models.User, as: 'users' }]
          });
        }
      ...
      const currentCountry = req.query['country'] || company.country || allActiveCountryCodes[0] || 'GB';

      const today = moment.utc();
      const bankHolidays = company.bank_holidays.filter(bh =>
        moment.utc(bh.date).year() === currentYear &&
        bh.country === currentCountry
      );
      ...
      res.render('bankHolidays', {
        ...
        currentCountry,
        countries: get_filtered_sorted_countries(allActiveCountryCodes),
      });
    });
});
...
const promiseNewBankHoliday = ({ model, req, companyId }) => {
  ...
  const country = req.body['country__new'] || req.body['country'] || req.user.company.country;

  return model.BankHoliday.create({ ...attributes, companyId, country });
};
...
router.post('/bankholidays/', function (req, res) {
  ...
  req.user.getCompany({
    ...
    .then(() => {
      return res.redirect_with_session(`/settings/bankholidays/?year=${currentYear}&country=${req.body['country'] || ''}`);
    });
});
...
router.post('/bankholidays/import/', (req, res) => {
  ...
  Promise
    ...
    .then(company => {
      ...
      const country_to_import = req.query['country'] || req.body['country'] || company.country || 'UK';

      // Fetch all default bank holidays known for current country
      let bank_holidays_to_import = config_countries[country_to_import].bank_holidays;
      ...
      bank_holidays_to_import = bank_holidays_to_import
        ...
        .map(
          bh => ({ name: bh.name, date: bh.date, companyId: company.id, country: country_to_import })
        );
    ...
    .then(() => {
      ...
      return res.redirect_with_session(`/settings/bankholidays/?year=${currentYear}&country=${country}`);
    });
});
