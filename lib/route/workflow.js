...
router.post('/workflow/watchers/update/', async (req, res) => {
    const db = req.app.get('db_model');
    const id = req.body.id;

    await db.WatcherRule.update({
        project_type: req.body.project_type,
        contract_type: req.body.contract_type || null,
        role_id: req.body.role_id || null,
        team_id: req.body.team_id || null,
    ...
    req.session.flash_message('Watcher rule updated');
    res.redirect_with_session('/settings/workflow/watchers/');
});

router.post('/workflow/watchers/delete/', async (req, res) => {
    const db = req.app.get('db_model');
    const id = req.body.id;

    await db.WatcherRule.destroy({
        where: { id, companyId: req.user.companyId }
    });

    req.session.flash_message('Watcher rule deleted');
    res.redirect_with_session('/settings/workflow/watchers/');
});
