...
const ApprovalResolver = require('../util/approval_resolver');
...
router.post('/bookleave/', function (req, res) {
  ...
  Promise.join(
    ...
    .then(leave => {
      ...
      const resolver = new ApprovalResolver(req.app.get('db_model'));
      return Promise.join(
        Email.promise_leave_request_emails({ leave }),
        resolver.resolveWatchers(leave).then(watchers =>
          Email.promise_watcher_notifications({
            leave,
            watchers,
            template_name: 'leave_request_to_watcher'
          })
        ),
        () => leave
      );
...
const groupUsersOnTeamViewByDepartments = (usersAndLeaves) => {
  const departmentsDict = usersAndLeaves.reduce(
    (acc, item) => {
      const deptId = (item.user && item.user.department) ? item.user.department.id : 'none';
      const deptName = (item.user && item.user.department) ? item.user.department.name : 'No Department';

      if (!acc[deptId]) {
        acc[deptId] = { departmentName: deptName, users_and_leaves: [] };
      }
      return acc;
    },
    {}
  ...
  usersAndLeaves.forEach(item => {
    const deptId = (item.user && item.user.department) ? item.user.department.id : 'none';
    departmentsDict[deptId].users_and_leaves.push(item);
  });
