...
var express = require('express'),
  ...
  ApprovalResolver = require('../util/approval_resolver');
...
router.get('/', function (req, res) {
  ...
  Promise.join(
    req.user
      ...
      .then(leaves => LeaveCollectionUtil.enrichLeavesWithUniqueApprovers({ leaves }))
    ...
    Promise.try(function () {
      ...
      .then(function (processed_leave) {
        ...
        return Promise.join(
          ...
          resolver.resolveWatchers(processed_leave).then(watchers =>
            Email.promise_watcher_notifications({
              leave: processed_leave,
              watchers,
              template_name: 'leave_request_decision_to_watcher',
              action: current_action
            })
          ),
          function () {
      ...
      return Promise.join(
        ...
        resolver.resolveWatchers(leave).then(watchers =>
          Email.promise_watcher_notifications({
            leave,
            watchers,
            template_name: 'leave_request_cancel_to_watcher'
          })
        ),
        function () {
