...
module.exports = function (sequelize) {
  ...
  this.promise_calendar = function (args) {
    ...
    return Promise.join(
      ...
      function (department, company, leaves, schedule) {
        ...
        const user_country = this_user.country || company.country || 'UK';
        ...
        return Promise.resolve(
          _.map(months_to_show, function (month) {
            return new CalendarMonth(
              ...
              {
                bank_holidays:
                  (department && department.include_public_holidays)
                    ? company.bank_holidays.filter(bh => bh.country === user_country)
                    : [],
  ...
  this.promise_my_leaves = function (args) {
    ...
    var promise_my_leaves = this.getMy_leaves({
      ...
      include: [{
        ...
        model: sequelize.models.ApprovalStep,
        as: 'approval_steps',
        include: [{
          model: sequelize.models.User,
          as: 'approver',
      ...
      order: [
        ['date_start', 'DESC'],
        [{ model: sequelize.models.ApprovalStep, as: 'approval_steps' }, 'sequence_order', 'ASC']
      ],
      where: where_clause,
    })
  ...
  //
  this.promise_leaves_to_be_processed = async function () {
    let self = this;
    const db = sequelize.models;

    // 1. Get leaves where current user is an explicit approver in ApprovalSteps
    const pendingSteps = await db.ApprovalStep.findAll({
      where: {
        approver_id: self.id,
        status: 1 // Pending
      },
      include: [{
        ...
        as: 'leave',
        where: {
          status: [db.Leave.status_new(), db.Leave.status_pended_revoke()]
        }
      }]
    });
    ...
    for (const step of pendingSteps) {
      ...
      // Check if all previous steps for this leave are approved
      const prevStepsQuery = {
        leave_id: leave.id,
        sequence_order: { $lt: step.sequence_order },
        status: { $ne: 2 } // Not approved
      };

      // Critical Fix: Scope by project_id if present
      // This ensures fully independent approval chains for different projects
      if (step.project_id) {
        prevStepsQuery.project_id = step.project_id;
      }

      const prevSteps = (step.sequence_order === null)
        ? []
        : await db.ApprovalStep.findAll({ where: prevStepsQuery });

      if (prevSteps.length === 0) {
        actionableLeaves.push(leave.id);
      }
    }

    // 2. Also include leaves where user is a supervisor (Legacy/Fallback)
    // ONLY if those leaves don't have any ApprovalSteps defined
    const supervisedUsers = await self.promise_supervised_users();

    const legacyLeaves = await db.Leave.findAll({
      where: {
        status: [db.Leave.status_new(), db.Leave.status_pended_revoke()],
        userId: supervisedUsers.map(u => u.id),
      }
    });

    for (const leave of legacyLeaves) {
      const hasSteps = await db.ApprovalStep.count({ where: { leave_id: leave.id } });
      if (hasSteps === 0) {
        actionableLeaves.push(leave.id);
      }
    }

    if (actionableLeaves.length === 0) {
      return [];
    }

    // Fetch full objects for actionable leaves
    const leaves = await db.Leave.findAll({
      ...
      where: { id: { $in: _.uniq(actionableLeaves) } },
    });
