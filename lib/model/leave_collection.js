...
const enrichLeavesWithUniqueApprovers = async ({ leaves }) => {
  leaves.forEach(leave => {
    if (leave.approval_steps && leave.approval_steps.length > 0) {
      // Deduplicate steps by approver ID
      // Keep the first one found (preserves order which usually prioritizes pending or sequence)
      leave.approval_steps = _.uniq(leave.approval_steps, step => step.approver.id);
    }
  });
  return leaves;
};
...
module.exports = function () {
  return {
    ...
    enrichLeavesWithUniqueApprovers,
