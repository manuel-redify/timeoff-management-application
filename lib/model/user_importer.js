...
const add_user_interface_schema = Joi.object().required().keys({
  ...
  contract_type: Joi.string().optional().default('Employee'),
  role_id: Joi.number().integer().optional().allow(null).allow(''),
  area_id: Joi.number().integer().optional().allow(null).allow(''),
  team_id: Joi.number().integer().optional().allow(null).allow(''),
...
function add_user(args) {
  ...
  attributes.country = args.country;
  attributes.end_date = args.end_date;
  attributes.contract_type = args.contract_type;
  attributes.DefaultRoleId = args.role_id;
  ...
  return Promise.resolve()
    ...
    .then(new_user => {
      ...
      const area_id = args.area_id;
      const team_id = args.team_id;
      const project_id = args.project_id;

      return Promise.all([
        new_user.setRoles(role_id ? [role_id] : []),
        new_user.setTeams(team_id ? [team_id] : []),
        // Manually handle UserProject association to include role_id
        Models.UserProject.destroy({ where: { user_id: new_user.id } })
          .then(() => {
            if (!project_id) return Promise.resolve();
            const projects = Array.isArray(project_id) ? project_id : [{ id: project_id, role_id: null }];
            return Promise.map(projects, p => Models.UserProject.create({
              user_id: new_user.id,
              project_id: p.id || p,
        ...
        if (role_id) {
          return Models.UserRoleArea.update(
            { area_id: area_id || null },
            { where: { user_id: new_user.id, role_id: role_id } }
          );
        }
      }).then(() => new_user);
    });
