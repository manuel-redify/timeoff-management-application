...
const
  ...
  scheme_constructor = Joi.object().required().keys({
    ...
    nominal_allowance: Joi.any().required(),
    now: schema_now,
  });
...
class UserAllowance {
  ...
  get nominal_allowance() {
    return this._nominal_allowance === null ? Infinity : this._nominal_allowance;
  }
  ...
  get employement_range_adjustment() {
    ...
    if (self.nominal_allowance === Infinity) {
      return 0;
    }
  ...
  static promise_allowance(args) {
    ...
    if (user.my_leaves === undefined || user.department === undefined || user.department === null) {
    ...
    flow = flow.then(() => {
      const args = {
        ...
        nominal_allowance: (user.department) ? user.department.allowance : null,
      };
