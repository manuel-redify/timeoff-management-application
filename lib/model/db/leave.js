  ...
  var Leave = sequelize.define("Leave", {
    ...
    classMethods: {
      associate: function (models) {
        ...
        Leave.hasMany(models.ApprovalStep, {
          as: 'approval_steps',
          foreignKey: 'leave_id',
        });
      },
    ...
    instanceMethods: {
      ...
      promise_to_reject: function (args) {
        ...
        // Find the relevant approval step for this user
        return self.getApproval_steps({
          where: { approver_id: by_user.id, status: Leave.status_new() }
        })
          .then(steps => {
            if (steps.length === 0) {
              // Fallback for legacy leaves or if no steps exist
            ...
            return Promise.map(steps, step => {
              step.status = Leave.status_rejected();
              return step.save();
            })
              .then(() => {
                // If one step is rejected, the whole leave is rejected (in most workflows)
                // For multi-project context, if one project rejects, the leave is rejected.
                self.status = self.is_pended_revoke_leave() ?
                  Leave.status_approved() :
                  Leave.status_rejected();
                self.approverId = by_user.id;
      ...
      promise_to_approve: function (args) {
        ...
        return self.getApproval_steps({
          ...
          .then(steps => {
            ...
            return Promise.map(steps, step => {
              step.status = Leave.status_approved();
              return step.save();
            })
              .then(() => {
              ...
              .then(all_steps => {
                let all_approved = _.every(all_steps, s => s.status === Leave.status_approved());
                ...
                if (all_approved) {
                  ...
                  // Keep status as new or current if not all approved
                  // We might want to trigger next sequential steps here
                }
                return self.save();
              });
          });
      ...
      promise_to_revoke: function () {
        return this.reload({
          ...
          .then(function (leave) {
            ...
            leave.approverId = (leave.user.department ? leave.user.department.bossId : null);
