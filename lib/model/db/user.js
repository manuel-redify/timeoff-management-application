...
module.exports = function (sequelize, DataTypes) {
  ...
  var User = sequelize.define("User", {
    ...
    contract_type: {
      ...
      comment: 'Type of contract: Employee or Contractor',
    },
    full_name: {
    ...
    DefaultRoleId: {
      ...
      comment: 'Default Role for the user',
    },
  }, {
    indexes: [
      {
...
function get_instance_methods(sequelize) {
  ...
  return {
    ...
    is_auto_approve: function () {
      return this.auto_approve === true || this.contract_type === 'Contractor';
    },
    ...
    promise_effective_role: function () {
      return this.getRoles()
        .then(roles => {
          if (!roles || roles.length === 0) return null;
          return roles.sort((a, b) => b.priority_weight - a.priority_weight)[0];
        });
    },

    promise_areas: function () {
      return this.getAreas();
    },

    promise_all_active_roles: async function (projectContext) {
      const self = this;
      const roles = [];
      let foundProjectRole = false;

      // 1. Get Project Role (if project context is provided)
      if (projectContext) {
        const UserProject = sequelize.models.UserProject;
        ...
        if (up && up.role) {
          roles.push(up.role);
          foundProjectRole = true;
        }
      }

      // 2. Get Default Role ONLY if no specific project role was found
      if (!foundProjectRole) {
        const defaultRole = await self.getDefault_role();
        if (defaultRole) {
          roles.push(defaultRole);
        }
      }

      return _.uniq(roles, r => r.id);
    },
    ...
    promise_supervisors: function () {
      return this.getDepartment({
        ...
        .then(department => {
          ...
          return _.flatten([department.boss, department.supervisors]);
        });
    },
