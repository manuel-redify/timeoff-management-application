...
Email.prototype.promise_leave_request_emails = function (args) {
  ...
  return Promise.all([
    ...
    .then(([comments, requesterAllowance, allPendingSteps]) => {
      // Find approvers of the first sequence order
      let approvers = [];
      if (allPendingSteps.length > 0) {
        const minOrder = _.min(allPendingSteps.map(s => s.sequence_order || 1));
        approvers = allPendingSteps
          .filter(s => (s.sequence_order || 1) === minOrder)
          .map(s => s.approver);
      }
...
Email.prototype.promise_leave_request_decision_emails = function (args) {
  ...
  return Promise.all([
    ...
    .then(([comments, requesterAllowance]) => {
      ...
      return leave.getApproval_steps({
        where: { status: 1 },
        include: [{ model: model.User, as: 'approver' }]
      })
        .then(nextPendingSteps => {
          if (nextPendingSteps.length > 0) {
            const minOrder = _.min(nextPendingSteps.map(s => s.sequence_order || 1));
            const actionableApprovers = nextPendingSteps
              .filter(s => (s.sequence_order || 1) === minOrder)
              .map(s => s.approver);
